...
for IN_LAYERS in 3 6 9; do
    for OUT_LAYERS in 3 6 9; do
        for ATTENTION in False True; do
            sbatch big_launcher.sh $MODEL $GUIDANCE_WEIGHT $DROP_PROB $DATASET_PORTION $PROPERTY $EPOCHS $TEST_EPOCHS $MODE $BATCH_SIZE $STABILITY_SAMPLES $EVAL_FLAG $IN_LAYERS $OUT_LAYERS $ATTENTION
        done
    done
done
